...
APAs will effectively not be used ??}
...
\fixme{Last sentence is not clear - try to reword.}

%The following paragraph may be cut if shorter text is needed, but I keep it here to show what
...
\fixme{"stitched across TPC's" is technical lingo and needs a better half sentence explanation.}
 The reconstructed
structure is described as a particle hierarchy using data products available in LArSoft.
\fixme{what are data products ??}
...
\fixme{add energy of pions to sentence}
\fixme{Add sentence to state result: e.g. spatial resolution of 0.6cm was found.}
