...
Extrapolating from our experience running Monte Carlo for previous DUNE MC campaigns,
...
\fixme{Thomas: why is this estimate still preliminary ? can't we used past experience again and the previously mentioned input assumptions ?}
-
\fixme{Maxim: this discussion is still ongoing even today, 9/8/2016. It is not clear what the performance will be of the reco software in a year
or two from now, and what data processing techniques we'll develop. On the other end of the spectrum, we may eventually choose to abandon
full reco for nearline/prompt processing.}
...
\fixme{This entire section needs to be rewritten: We can't have a laundry list of potential options in a TDR. Instead we need to clearly state
what the requirements are. Then pick a baseline design/system that meets the requirements from the list of options and describe it. If there are alternates describe them as such. As written it gives the impression that we are just starting to think about possible ways to implement this which is not appropriate for a TDR.  }
