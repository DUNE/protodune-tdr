...
%The right-hand image in the figure shows a top and bottom FC attached only to one side of the CPA. % module.  
...
\end{cdrfigure}
\fixme{In the text it says right side image shows CPA pair, in caption the correction implies that it shows the FC. I can't tell which. Anne}
...
\begin{itemize}
\item After the first row of APAs is installed and translated to the Sal\`{e}ve (south) side of the cryostat, the beam \fixme{which beam?} is bolted into place. The two end walls for the Sal\`{e}ve-side drift volume will be constructed and moved inside the cryostat supported 
...
\end{itemize}
\fixme{Anne updated the bullet points per Bill Miller 2/16/17}
