...
A cryogenics system maintains the LAr at a stable temperature of about 89\,K and at the required purity level \fixme{add ``of (value)''} through a closed-loop process that recovers the evaporated argon, recondenses and filters it, and returns it to the cryostat. 
...
layer of tetraphenyl-butadiene (TPB) that converts incoming VUV (128 nm) %Ar 
...
Each APA frame is designed with ten bays into which PDS modules are inserted after the TPC wires have been strung. This  allows final assembly at the integration area (in a clean room) at the CERN NP prior to installation inside the cryostat. 
