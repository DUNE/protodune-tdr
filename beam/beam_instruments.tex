...
\fixme{$X_0$ mentioned in sec 4.9.7 but not defined anywhere; is it a radiation length or something?} The tree \fixme{three?} devices needed for momentum measurement will consist of one layer only, oriented perpendicularly to the magnet deflection.
...
Scintillation light will be read out by SiPMs, which in turn will be read out by custom electronics depending on the application. % (see below about the use for Time of Flight).  <--- I'd leave this out
%
...
\fixme{predict?}  a resolution better than 1~ns. 
...
\fixme{could we rephrase to ``configuration of the beamline at different beam particle momenta''? (Anne)}
