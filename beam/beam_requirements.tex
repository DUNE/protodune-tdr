...
The parameters defining this beam are primarily driven by the requirement that these test beam results be directly applicable to DUNE's future large underground single-phase detector module(s) with minimal extrapolation.
...
The expected momentum distributions for secondary particles from neutrino interactions, shown in Figure~\ref{fig:particlemomenta}, exhibit a large spread with a clear peak near 200~MeV/c per particle. 
The desirable range for ProtoDUNE-SP is in the low-momentum region. Based on the feedback and constraints from the CERN beam group, the beamline design allows the transport of beam particles from about 0.5 GeV/c up to 7 GeV/c. \fixme{this last sentence doesn't flow well from the one before it. Anne}
...
\fixme{is this like a bunch rate? Not sure what `particle rate' actually means. Anne}
