...
Ten stacks of wire boards are installed across the width of each side along the head of the APA.  The X-layer board in each stack has room for 48 wires, the V layer has 40 wires, the U layer 40 wires and the G layer 48 wires.  Each board stack, therefore, has 176 wires but only only 128 signal channels since the G wires are not read out.  
...
\fixme{Eric has question on `longitudinal' is it vertical X and G plane?}
...
Some wire segments are quite long; for instance, the longitudinal wires 
\fixme{Eric has question on `longitudinal' is it vertical X and G plane?}
...
Putting the electronics in small boxes simplifies installation and replacement, and also helps with the collection 
\fixme{dissipation?}
