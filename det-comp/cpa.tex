...
\fixme{Need to clarify language: One CPA plane (7m x 6m) = 1 CPA, made up of six CPA columns, each CPA column made up of 3 vertically stacked CPA modules. Each module made up of resistive panel in frame. Anne to do.}
...
\item support the full weight of the four connected top/bottom FC modules plus a person on the bottom CPA \fixme{not FC?} during installation,
...
Among the possible solutions, the DUNE-SP TPC design has adopted one in which the entire cathode plane is made out of highly resistive material such that it has a very long discharge time constant compared to an all metal construction.  In the event of HV breakdown at a given location on the cathode plane, the sudden change in voltage is restricted to the CPA module in question, \fixme{true?} a relatively localized area.  The rest of the CPA maintains its original bias voltage, and gradually discharges to ground through the large resistivity of the cathode material.  This greatly reduces the instantaneous charge injection to the front-end electronics.
...
%Compared with the size of an APA, the CPA modules are 1/2 in wide (1.16\,m) and 1/3 in high (2\,m).  
...
%Instead, each adjacent pair is interconnected by a ``high-voltage bus''.  \fixme{this must be wrong - there must be a bus only on the outer edges of the outer columns}
%
...
The HV bus makes redundant connections to the resistive panels across CPA columns. \fixme{but only at the top and bottom?}  It also provides a low-resistance path for the FC resistive divider chains around the cathode edges.
