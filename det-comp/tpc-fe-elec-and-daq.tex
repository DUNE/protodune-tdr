...
\begin{cdrfigure}[The front-end electronics mounted on an APA]{tpcce_CMBonAPA}{The 
...
\includegraphics[width=1.0\linewidth]{tpcce_apa_flange.pdf}
\end{cdrfigure}
...
fiber to the DAQ (RCE or FELIX) described in Section~\ref{daq-stuff}.

\fixme{Reference to DAQ section needed.}
...
\fixme{We should include the noise and gain measurements from the latest FEMB tests at the last collab meeting.}
...
replaced by an FPGA mezzanine (Figure~\ref{tpcce_CMBpix}) 
...
each of these feedthroughs will serve the bias/power/digital IO needs of one APA, as shown in Figure~\ref{tpcce_cable_routing}.  

\begin{cdrfigure}[TPC]{tpcce_cable_routing}{TPC cable routing scheme.}
...
\label{subsec:ce_install_assembly}
