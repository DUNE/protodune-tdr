...
\fixme{List components: timing, trigger, throttle systems, FPGA-based master, etc.}
...
synchronization signals, and a general-purpose packet data output \fixme{ packet of data output?} to each endpoint.
...
an electron beam trigger.  If possible, a $\pi$/K/p will also be added.
...
Data from the beam
instrumentation are acquired continuously via a separate DAQ path. \fixme{separate from the actual data?}  Triggered
...
Boards), manipulates the data, and delivers them to the back-end DAQ.
...
\fixme{This could use a bit of introduction, so I'm moving things around a bit, e.g., start with}
...
responsible for this.  \fixme{prev sentence is awkward}
