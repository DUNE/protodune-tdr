...
LV supply modules          &  6, 1 per APA       &  128 $\times$ 20            \\
Wire-bias mini-crate               & 2                        &   7,680   \\    \colhline
Wire-bias supply modules    & 6, 1 per APA                        &   128 $\times$ 20   \\   \colhline
\end{cdrtable}
...
on the 7 meter LV cables to each FEMB is ~0.1W at room temperature, or ~2W per APA, and will be 
...
electronics connected through the signal flange, approximately 250~W at 48~V for a
fully-loaded flange (one~PTC, five~WIB, and 20~FEMB). The LV power is then stepped down
to 12~V via a DC/DC converter onboard the PTC and fanned out
...
(one fiber pair per FEMB) to two RCEs. \fixme{write out} The FPGA has an additional Gbps Ethernet
...
requires 12~V and draws approximately 20~Amps, for a total power of almost 250~W, as 
...
operate at 30-60V/13.5A/650W maximum capacity. Using 10~AWG cable, a .8V drop is expected along the cable with a
...
%\includegraphics[width=0.7\linewidth]{tpcce_power_connector.png} 
\end{cdrfigure}
...
The remaining five wire-bias voltage lines supply 1$\sim$1.5~kV to the field cage terminations (3)
