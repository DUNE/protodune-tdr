...
shown in Figure~\ref{tpcce_cr_board}.
...
and follow the grounding guidelines in the next section.
...
it is used to qualify electronics for deep space NASA missions as well as commercial PCs.
...
Assuming a conservative serial-link transmission speed of $\sim$1.2~Gbps, 
a single link can therefore handle 32 channels.
Thus, it is planned to drive four $\sim$1.2~Gbps links from each COLDATA ASIC.
...
which will reproduce the clock/control IO, data multiplexing, and $\sim$1.2~Gbps serial drivers.
...
Except for some small adjustments as required for integration with the cryostat, expected to be complete by October 2016,
the design for the signal feedthroughs is complete.
...
\includegraphics[width=6in]{tpcce_samtec_data.pdf}
\includegraphics[width=6in]{tpcce_samtec_lv.pdf}
\end{cdrfigure}
