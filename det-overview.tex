...
insert detector overview here \fixme{The whole chapter is `overview'; insert detector requirements here!}
...
\fixme{Do we want to avoid first person ``we''? It occurs several times}
...
applying an appropriate bias voltage to these wire planes. During this process, \fixme{During what `process'?}
...
with the first two being induction and last one being collection plane.  \fixme{Sorry, figure is for one induction plane, but of the three, two are induction? Please clarify}
...
\begin{cdrfigure}[The process of TPC signal formation.]{signal_formation}{The process of TPC signal formation.}
\includegraphics[width=0.8\textwidth]{Signal_Extraction.png}
\end{cdrfigure}
\fixme{Figure could use more explanation of orange (E field?) and green (weighting field?) lines. Also indicate `boundaries'; make sure vocabulary squares with that in the text.}
...
\includegraphics[width=0.8\textwidth]{electronics_res.pdf}
\end{cdrfigure}
...
will drift nearest to its associated wire. \fixme{drift to its nearest associated wire?}
...
\fixme{within the wire's boundary? Please use same term for same thing each time.} the expected 
...
\fixme{I think you mean Figure~\ref{fig:ideal_track_1}}
...
\begin{cdrfigure}[Induction plane field response for an ideal track (1)]{ideal_track_1}{Illustration of the induction plane field response for an ideal track (red arrow).}
  \includegraphics[width=0.8\textwidth]{ideal_track.png}
\end{cdrfigure}
...
\includegraphics[width=0.95\textwidth]{ideal_track_2.png}
\end{cdrfigure}
